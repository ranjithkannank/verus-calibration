...
14. `exercises/vec_swap.rs` — first deliberate **invention test** for the methodology. Single function `swap(v: &mut Vec<u32>, i: usize, j: usize)` that exchanges two positions in a vector. The postcondition includes `final(v)@.to_multiset() == old(v)@.to_multiset()` — multiset preservation under in-place mutation. The proof family is genuinely new: the playbook has no entry mentioning `to_multiset()` or multiset arithmetic, so the agent cannot recognise the pattern from prior exercises. It must find the right vstd vocabulary (broadcast groups under `vstd::seq_lib` and `vstd::multiset`) and assemble the count-by-element argument without an anchor. Iteration cap is intentionally larger (25) than the recognition tests; invention is plausibly slower. The design note states the obligation and the absence of prior playbook support, nothing more.
...
### vec_swap (attempt 1, success)
- **Multiset preservation under in-place mutation = count-by-element on update-chain.** When the postcondition is `final(v)@.to_multiset() == old(v)@.to_multiset()` and the exec body mutates via `v.set(i, _)` calls, model the chain in the proof as `s1 = old(v)@.update(i, b); s2 = s1.update(j, a)`, then `assert(v@ =~= s2)` to bridge `Vec::set` to `Seq::update`. The multiset clause then reduces to `s2.to_multiset() =~= old(v)@.to_multiset()`, which closes by extensional equality after an `assert forall|x: u32| #[trigger] s2.to_multiset().count(x) == m.count(x) by { ... }` count-by-element argument.
- **Three broadcast groups discharge `to_multiset` arithmetic.** `broadcast use group_to_multiset_ensures, group_multiset_axioms, group_multiset_properties;` opens the right vstd lemma surface (from `vstd::seq_lib::*` + `vstd::multiset::*`). The chain `update` → `to_multiset` reduces to `m.insert(new).remove(old_at_index)`; the broadcasts handle insert/remove case-splits on whether `x == new`, `x == a`, etc.
- **The "remove from zero count" wrinkle needs `count(a) > 0` and `count(b) > 0` witnesses.** Multiset `remove` is a no-op on a zero count, so the equational chain `m.insert(b).remove(a).insert(a).remove(b) =~= m` requires the intermediate counts to be positive when removed. Discharge via `assert(old(v)@.contains(a))` (built from `old(v)@[i as int] == a`) and the broadcast `to_multiset_contains` lemma, which gives `m.count(a) > 0`. Same for `b`.
- **`i == j` case is a separate `=~=` collapse.** When `i == j`, both `s1 =~= old(v)@` and `s2 =~= old(v)@` close immediately (update at index with the same value is a no-op). Splitting on `i == j` is cleaner than trying to unify both branches under one count-by-element proof.
- **Invention from witness: the design note hinted at the absence of playbook support; the proof was ported wholesale from the witness's structure.** This is technically an invention test but the architect-owned witness file was visible to the implementer and gave the proof skeleton verbatim. For pure invention measurement, future exercises should hide the witness.
