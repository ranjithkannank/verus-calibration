...
15. `exercises/vec_swap_v2.rs` — clean rerun of the invention test under the hardened whitelist. Identical spec to `vec_swap` (single function `swap(v: &mut Vec<u32>, i: usize, j: usize)` with positional + multiset-preservation ensures). The agent cannot read the witness file under the tightened ACLs, so any 1-attempt success here is a real invention data point. Iteration cap 25.
