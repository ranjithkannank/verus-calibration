// `fusion` module for the sensor_poll_signed exercise.
//
// Byte-identical to `exercises/sensor_poll/fusion.rs`. Direct port
// of the marzullo exercise. The spec (the type aliases, the uninterp
// spec fn, the five open spec fns, and `marzullo`'s signature with
// requires/ensures) is FROZEN. The implementer ports the verified
// body from `exercises/sensor_poll/fusion.rs` (or `exercises/marzullo.rs`)
// and writes any helper lemmas needed.
//
// Reference: see `exercises/sensor_poll/fusion.rs` for the verified
// body and proof helpers — they carry over verbatim. The agent may
// read that file freely.
